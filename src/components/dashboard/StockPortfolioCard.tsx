...
function KRWFlashCard({
  ...
  return (
    <div className='absolute inset-0 bg-white z-10 flex flex-col justify-center px-4 py-3 rounded-inherit'>
      {/* 상단: 이름 + 닫기 */}
      <div className='flex items-center justify-between mb-2'>
        <div className='flex items-center gap-1.5 min-w-0'>
          ...
          <span className='text-xs font-semibold text-slate-700 truncate'>원화 환산</span>
      ...
      <div className='flex items-center gap-2'>
        ...
        <div className='flex-1 min-w-0'>
          <p className='text-[10px] text-slate-400 mb-0.5'>보유금액</p>
          <p className='text-base font-black text-slate-900 tabular-nums leading-tight'>{formatKRW(valueKRW)}</p>
        </div>
        ...
        {/* 매수금액 */}
        <div className='flex-1 min-w-0'>
          <p className='text-[10px] text-slate-400 mb-0.5'>매수금액</p>
          <p className='text-sm font-bold text-slate-600 tabular-nums leading-tight'>{formatKRW(costKRW)}</p>
        </div>
        ...
        <div className='flex-1 min-w-0 text-right'>
          <p className={`text-[10px] mb-0.5 font-semibold ${isUp ? 'text-emerald-500' : 'text-rose-400'}`}>
            {isUp ? '수익' : '손실'}
          </p>
          <p className={`text-sm font-bold tabular-nums leading-tight ${isUp ? 'text-emerald-700' : 'text-rose-600'}`}>
            {isUp ? '+' : ''}{formatKRW(pnlKRW)}
          </p>
          <p className={`text-[10px] tabular-nums ${isUp ? 'text-emerald-500' : 'text-rose-400'}`}>
